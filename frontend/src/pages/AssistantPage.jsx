          ...
          <div className="mt-1.5 bg-white border border-surface-3 rounded-xl overflow-hidden max-w-sm">
            {message.data.slice(0, 8).map((item, i) => (
              <div key={i} className={`flex items-center gap-2 px-3 py-2 text-xs ${
                i < message.data.length - 1 && i < 7 ? 'border-b border-surface-3' : ''
              }`}>
...
export default function AssistantPage() {
  ...
  async function handleSend(text) {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        text: `Lo siento, ha ocurrido un error: ${err.message}. Verifica que el backend esté activo.`,
        timestamp: Date.now(),
      }]);
    } finally {
      setLoading(false);
      ...
      switch (action) {
        case 'create_channel': {
          ...
          const { error } = await supabase.from('channels').insert({
            ...
            address: params.address || null, city: params.city || null,
            ...
            for (const ch of (allChannels || [])) {
              ...
              if (!lastVisit?.length || lastVisit[0].checkin_at < threshold) {
                const days = lastVisit?.length
                  ? Math.floor((Date.now() - new Date(lastVisit[0].checkin_at).getTime()) / 86400000) : null;
        ...
        case 'move_pipeline': {
          ...
          const { data: channels } = await supabase.from('channels').select('id, name, pipeline_stage')
            .eq('assigned_to', user.id).ilike('name', `%${params.channel_name}%`).limit(1);
          if (!channels?.length) return { actionResult: `No encontré "${params.channel_name}"`, actionSuccess: false };
        ...
        case 'plan_visit': {
          ...
          const { error } = await supabase.from('planned_visits').insert({
            channel_id: channels[0].id, kam_id: user.id, planned_date: date,
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 140px)' }}>
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-hide py-2">
        {['¿Cuántas visitas llevo esta semana?', '¿Qué canales no he visitado?', 'Mis estadísticas', 'Lista de canales activos'].map((s, i) => (
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-surface-3">
        <button onClick={toggleVoice}
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
            listening ? 'bg-red-500 text-white animate-pulse' : 'bg-surface-2 text-text-muted hover:bg-surface-3'
          }`}>
        ...
        <button onClick={() => handleSend()} disabled={!input.trim() || loading}
          className="w-10 h-10 rounded-xl bg-brand-500 hover:bg-brand-600 disabled:opacity-30 text-white flex items-center justify-center flex-shrink-0">
          <Send size={16} />
        </button>
      </div>
    </div>
