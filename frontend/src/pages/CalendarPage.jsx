...
import {
  ...
  Clock, MapPin, Building2, CalendarDays, Mail
} from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div>
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Buscar canal..." className={`${fieldClass} mb-2`} />
              ...
              {filtered.map(ch => (
                <button key={ch.id} onClick={() => { setSelectedChannel(ch.id); setSearch(ch.name); }}
                  className={`w-full flex items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors ${
                    selectedChannel === ch.id ? 'bg-brand-500/10 text-brand-400' : 'hover:bg-surface-1 text-text-primary'
                  }`}>
          ...
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              placeholder="Objetivo de la visita, preparación..." rows={2} className={`${fieldClass} resize-none`} />
          </div>
        </div>
...
function PlannedVisitCard({ visit, onDelete }) {
  const time = visit.planned_time ? visit.planned_time.slice(0, 5) : '--:--';
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
      visit.is_completed ? 'bg-green-500/5 border-green-500/20' : 'bg-surface-1 border-surface-3'
    }`}>
      <div className="text-center flex-shrink-0 w-12">
        <div className={`text-sm font-bold ${visit.is_completed ? 'text-green-400' : 'text-text-primary'}`}>{time}</div>
      </div>
      <div className={`w-0.5 h-10 rounded-full flex-shrink-0 ${visit.is_completed ? 'bg-green-500' : 'bg-surface-3'}`} />
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-text-secondary truncate">{visit.channels?.address || 'Sin dirección'}</div>
        {visit.notes && <div className="text-[10px] text-text-muted mt-0.5 truncate">{visit.notes}</div>}
      </div>
      <div className="flex items-center gap-1 flex-shrink-0">
        {visit.is_completed ? (
          ...
          <>
            {visit.email_sent && <Mail size={12} className="text-brand-400" title="Email enviado" />}
            <button onClick={(e) => { e.stopPropagation(); onDelete(visit.id); }}
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    if (user) { loadWeekData(); loadChannels(); }
  ...
  async function loadChannels() {
    const { data } = await supabase.from('channels').select('id, name, address').eq('assigned_to', user.id).order('name');
    setChannels(data || []);
  }
  ...
  async function handleSavePlanned(data) {
    try {
      const { data: saved, error } = await supabase.from('planned_visits').insert({
        ...data,
        kam_id: user.id,
      }).select('*, channels(name, address)').single();
      if (error) throw error;

      // Enviar email de invitación al KAM
      ...
      try {
        await fetch(`${BACKEND_URL}/api/calendar-invite`, {
          ...
          body: JSON.stringify({
            kam_email: profile?.email,
            kam_name: profile?.full_name,
            channel_name: channelData?.name || 'Canal',
            channel_address: channelData?.address || '',
            planned_date: data.planned_date,
            planned_time: data.planned_time,
            notes: data.notes || '',
          }),
        });
        setToast({ message: '✓ Visita planificada · Email enviado', type: 'success' });
      } catch (emailErr) {
        console.error('Error enviando email:', emailErr);
        setToast({ message: '✓ Visita planificada (email no enviado)', type: 'success' });
      }
  ...
  const dayCompleted = completedVisits.filter(v => isSameDay(new Date(v.checkin_at), selectedDay));
    ...
    <div className="space-y-4">
              ...
              {counts.total > 0 && (
                <div className="flex gap-0.5 mt-1">
                  {Array.from({ length: Math.min(counts.total, 4) }).map((_, j) => (
                    <div key={j} className={`w-1 h-1 rounded-full ${j < counts.completed ? 'bg-green-400' : 'bg-brand-400'}`} />
                  ))}
                </div>
              )}
            </button>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] text-text-secondary">{dayPlanned.length} planificadas · {dayCompleted.length} realizadas</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-surface-1 border border-surface-3 rounded-xl">
            ...
            <button onClick={() => setShowNewModal(true)}
              className="mt-3 text-xs font-semibold text-brand-400 hover:text-brand-300 transition-colors">+ Planificar visita</button>
          </div>
        ) : (
          <div className="space-y-2">
            {dayPlanned.map(visit => <PlannedVisitCard key={visit.id} visit={visit} onDelete={handleDeletePlanned} />)}
            {dayCompleted.filter(cv => !dayPlanned.some(pv => pv.visit_id === cv.id)).map(visit => {
              ...
              return (
                <div key={visit.id} className="flex items-center gap-3 p-3 rounded-xl bg-green-500/5 border border-green-500/20">
                  <div className="text-center flex-shrink-0 w-12"><div className="text-sm font-bold text-green-400">{time}</div></div>
      ...
      <div className="bg-surface-1 border border-surface-3 rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div><div className="text-lg font-extrabold text-brand-400">{plannedVisits.length}</div><div className="text-[9px] text-text-muted uppercase tracking-wider">Planificadas</div></div>
          <div><div className="text-lg font-extrabold text-green-400">{completedVisits.length}</div><div className="text-[9px] text-text-muted uppercase tracking-wider">Realizadas</div></div>
          <div><div className="text-lg font-extrabold text-amber-400">{Math.max(0, plannedVisits.filter(v => !v.is_completed && new Date(v.planned_date) <= today).length)}</div><div className="text-[9px] text-text-muted uppercase tracking-wider">Pendientes</div></div>
        </div>
      </div>

      {showNewModal && (
        <NewPlannedVisitModal date={selectedDay} channels={channels} onSave={handleSavePlanned} onClose={() => setShowNewModal(false)} />
      )}
